...
class DetailBranch(BaseModule):
    ...
    def forward(self, x):
        ...
        z=torch.add(y_1,y_2)
        list[0]=self.add_branch[2](list[0])#256x256,64
        list[1] = self.add_branch[3](list[1])#128x128,64
        list[2] = self.add_branch[4](list[2])#64x64,128
...
class BiSeNetV2(BaseModule):
    ...
    def __init__(self,
        ...
        self.dysample = DySample(128,scale=4)
    def forward(self, x):
        #  stole refactoring code from Coin Cheung, thanks
        #x=self.sm(x)
        x_detail,z_detail,list = self.detail(x)
        x_semantic_lst,z_semantic_lst = self.semantic(x)
        x_datail_final=torch.add(x_detail,z_detail)
        x_detail=torch.add(z_semantic_lst,x_datail_final)
        #x_head = self.bga(x_detail, x_semantic_lst[-1])
        z= x_semantic_lst[-1]
        z=self.dysample(z)
        x_head=torch.add(x_detail, z)
        outs = [x_head] + list[:3]

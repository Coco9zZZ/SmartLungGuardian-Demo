...
class DetailBranch(BaseModule):
    ...
    def __init__(self,
        ...
        self.maxpool = torch.nn.AdaptiveMaxPool2d((1, 1), return_indices=False)
    ...
    def forward(self, x):
        ...
        y_1=self.add_branch[0](y_1)
        y_2 = self.add_branch[1](y_2)
        y_3=self.add_branch[2](x)
        cat_y=torch.cat((y_1,y_2,y_3),dim=1)
        cat_maxpool=self.maxpool(cat_y)
        cat_avgpool=self.avgpool(cat_y)
        #sigmod=torch.sigmoid()
        cat_maxpool=torch.sigmoid(cat_maxpool)
        cat_avgpool=torch.sigmoid(cat_avgpool)
        cat_maxpool=self.BN(cat_maxpool)
        cat_avgpool=self.BN(cat_avgpool)
        z=torch.add(cat_maxpool,cat_avgpool)
        chunk=torch.chunk(z,chunks=3,dim=1)
        y_1_c=y_1 * chunk[0]
        y_2_c = y_2 * chunk[1]
        y_3_c = y_3 * chunk[2]
        z=torch.cat((y_1_c,y_2_c,y_3_c),dim=1)
        # x_left_c=x_left*chunk[0]
        # x_right_c = x_right * chunk[1]
        list[0]=self.add_branch[3](list[0])#256x256,64
        list[1] = self.add_branch[4](list[1])#128x128,64
        list[2] = self.add_branch[5](list[2])#64x64,128
...
class StemBlock(BaseModule):
    ...
    def forward(self, x):
        ...
        x_left = self.convs(x)
        #x_right = self.pool(x)
        x_right=self.spdconvs(self.spd(x))
        z=torch.cat((x_left,x_right),dim=1)
        z=self.maxpool(z)
        #sigmod=torch.sigmoid()
        z=torch.sigmoid(z)
        z=self.BN(z)
        chunk=torch.chunk(z,chunks=2,dim=1)
        x_left_c=x_left*chunk[0]
        x_right_c = x_right * chunk[1]
        new=torch.add(x_left_c,x_right_c)
...
class SemanticBranch(BaseModule):
    ...
    def forward(self, x):
        ...
        cat_y=torch.cat((x_4,x_8,x_16),dim=1)
        cat_maxpool=self.maxpool(cat_y)
        cat_avgpool=self.avgpool(cat_y)
        #sigmod=torch.sigmoid()
        cat_maxpool=torch.sigmoid(cat_maxpool)
        ...
        x_8 = x_8 * chunk[1]
        x_16 = x_16 * chunk[2]
        z=torch.cat((x_4,x_8,x_16),dim=1)
        a=0.5
        z=a*z
...
class BGALayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.dys=DySample(128,scale=4)
        self.up=nn.Upsample(size=(64, 64))

    def forward(self, x_d, x_s):
        x_d_dw = x_d
        x_d_maxpool = self.maxpool(x_d)
        x_d_avgpool = self.avgpool(x_d)
        # sigmod=torch.sigmoid()
        x_d_maxpool = torch.sigmoid(x_d_maxpool)
        x_d_avgpool = torch.sigmoid(x_d_avgpool)
        x_d_maxpool = self.BN(x_d_maxpool)
        x_d_avgpool = self.BN(x_d_avgpool)
        x_d_w=torch.add(x_d_maxpool,x_d_avgpool)

        x_s_maxpool = self.maxpool(x_s)
        x_s_avgpool = self.avgpool(x_s)
        x_s_maxpool = torch.sigmoid(x_s_maxpool)
        x_s_avgpool = torch.sigmoid(x_s_avgpool)
        x_s_maxpool = self.BN(x_s_maxpool)
        x_s_avgpool = self.BN(x_s_avgpool)
        x_s_w=torch.add(x_s_maxpool,x_s_avgpool)
        r=torch.cosine_similarity(x_s_w,x_d_w)
        r = torch.unsqueeze(r, dim=1).expand(-1, 128, -1, -1)
        #print(r.shape)
        x_s_w=x_s_w*(1-r)
        x_d_w=x_d_w*r

        x_s_D=self.up(x_s)
        x_d_w_r=x_d_dw*x_d_w
        x_s_w_r = x_s_D * x_s_w
        #x_s_w_r=DySample(x_s_w_r)
        x_d_f=torch.add(x_s_w_r,x_d_dw)
        x_s_f = torch.add(x_d_w_r, x_s_D)
        output=torch.add(x_d_f,x_s_f)
        #z = torch.cat((y_1_c, y_2_c, y_3_c), dim=1)
...
class BiSeNetV2(BaseModule):
    ...
    def __init__(self,
        ...
        self.add1 = nn.Conv2d(in_channels=192, out_channels=128, kernel_size=1, stride=1)
    def forward(self, x):
        #  stole refactoring code from Coin Cheung, thanks
        #x=self.sm(x)
        x_detail,z_detail,list = self.detail(x)
        x_semantic_lst,z_semantic_lst = self.semantic(x)

        new=torch.add(z_semantic_lst,z_detail)
        x_datail_final = self.add1(new)
        x_detail=torch.add(x_detail,x_datail_final)

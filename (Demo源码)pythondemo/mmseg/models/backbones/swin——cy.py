...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        # self.add1 = nn.Conv2d(in_channels=96, out_channels=128, kernel_size=1, stride=1)
        # self.add2 = nn.Conv2d(in_channels=192, out_channels=16, kernel_size=1, stride=1)
        # self.add3 = nn.Conv2d(in_channels=384, out_channels=32, kernel_size=1, stride=1)
        # self.add4 = nn.Conv2d(in_channels=768, out_channels=64, kernel_size=1, stride=1)
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            self.fuss_branch.append(
                nn.Sequential(
                    DepthwiseSeparableConvModule(
                        ...
                        pw_norm_cfg=None,
                        pw_act_cfg=None), DepthwiseSeparableConvModule(
                        in_channels=256,
                        out_channels=96,
                        kernel_size=3,
                        stride=1,
            ...
            self.conv1=torch.nn.Conv2d(in_channels=1,out_channels=1,kernel_size=1,stride=1)
            self.spd=space_to_depth()
            self.conv2=torch.nn.Conv2d(in_channels=1,out_channels=1,kernel_size=1,stride=1)
    def forward(self, x):
        x1=x
        x2=x
        self_RS = ResNeXt(depth=50)
        self_RS= self_RS.to('cuda')
        outputs_1 = self_RS.forward(x1)
        RSlist=[]
        for i in outputs_1:
            RSlist.append(i)
            print('outputs_1', i.shape)
        self_Rx_ECA = ResNeXt_eca(depth=50)
        self_Rx_ECA = self_Rx_ECA.to('cuda')
        outputs_2 = self_Rx_ECA.forward(x2)
        RxECAList=[]
        for i in outputs_2:
            RxECAList.append(i)
            print('outputs_2', i.shape)
        x, hw_shape = self.patch_embed(x)
        print('x_patch_em',x.shape)
        ...
        print(outs)
        list=[]
        for i,j,k in RSlist,RxECAList,outs:
            f1=torch.add(i,j)
            f2=torch.cat(f1,k,dim=1)
            list.append(f2)
        fuss_list=[]
        for i in range(4):
            fuss = self.fuss_branch[i](list[i])
            fuss_list.append(fuss)
        inlist_1=[768,384,192]
        inlist_2=[96,192,384]
        uplist=[] #[384,192,96]
        for j in range(3):
            z=self.conv1(self.up(fuss_list[3-j]),inlist_1[j],inlist_1[j]/2)
            f=torch.add(z,fuss_list[2-j])
            uplist.append(f)
        downlist=[uplist[2]] #[96,192,384,768]
        for k in range(2):
            z=self.conv2(self.down(downlist[k]),inlist_2[k],inlist_2[k]*2)
            f=torch.add(z,uplist[k+1])
            downlist.append(f)
        z=self.conv2(self.down(downlist[2]),384,768)

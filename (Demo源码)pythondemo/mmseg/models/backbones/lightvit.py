...
class LightViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[32, 64, 160, 256],
                 num_layers=[2, 2, 2, 2],
                 num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], num_tokens=8, window_size=7, neck_dim=1280,
                 qkv_bias=True,
        ...
        for stage, (embed_dim, num_layer, num_head, mlp_ratio) in enumerate(
                zip(embed_dims, num_layers, num_heads, mlp_ratios)):
            blocks = []
            if stage > 0:
                # downsample
            ...
            blocks += [
                Block(
                    dim=embed_dim, num_heads=num_head, num_tokens=num_tokens, window_size=window_size,
                    mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=dpr[sum(num_layers[:stage]) + i], norm_layer=norm_layer,
                    act_layer=act_layer, attention=Attention,
